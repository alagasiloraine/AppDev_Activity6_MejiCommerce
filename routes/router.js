...
router.get('/', (req, res) => {
    return res.render('home', {
        user: req.session.user || null  // Provide a fallback if user is not logged in
    });
});
...
router.get('/admin/customers', adminController.getUsers);
// Admin routes (Handled by adminController)
router.get('/admin/dashboard', adminController.getDashboardData);

// Admin routes for category (Handled by adminController)
router.get('/admin/category', adminController.getCategories);  // Get all categories
router.post('/admin/category', adminController.addCategory);  // Add a new category
// Add this new route for deleting categories
router.post('/admin/category/:id/delete', adminController.deleteCategory); // Delete a category by ID

// Admin routes for adding a product
router.get('/admin/products', adminController.getProducts); // Ensure that 'adminController.getProducts' is correctly defined
